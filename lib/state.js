...
function setWindowStateSync(ws) {
  const home = docketHome();
  fsSync.mkdirSync(home, { recursive: true });
  let current;
  try {
    current = JSON.parse(fsSync.readFileSync(statePath(), 'utf8'));
  } catch {
    current = defaultState();
  }
  const next = { ...current, windowState: ws };
  fsSync.writeFileSync(statePath(), JSON.stringify(next, null, 2) + '\n');
}
...
module.exports = { read, write, addRecent, removeRecent, addFavorite, removeFavorite, setSortBy, setSortReverse, setSearchMode, setDocScale, setWindowState, setWindowStateSync, setSectionOrder, setSectionCollapsed, setFavoritesOrder, setTabs, setActiveTabIndex, setAutoCheck, setAllowPrerelease, setLastUpdateCheck, setDefaultView, setActiveBrowseRoot, setVoiceURI, setSpeechRate, setSidebarWidth, DOC_SCALE_MIN, DOC_SCALE_MAX, SPEECH_RATE_MIN, SPEECH_RATE_MAX, SIDEBAR_WIDTH_MIN, SIDEBAR_WIDTH_MAX, VALID_SECTIONS, DEFAULT_SECTION_ORDER, VALID_DEFAULT_VIEWS };
